...
class Account {
    ...
    async claims(use, scope, claims, rejected) { // eslint-disable-line no-unused-vars
        ...
        // Namespaces the caller may access, from the external enrichment
        // webhook. Mirrors the JWT-access-token path in configuration.js so the
        // same claim reaches id_token/userinfo consumers (e.g. the driftmower
        // BFF). Fail-open: fetchExtraClaims returns {} on any error.
